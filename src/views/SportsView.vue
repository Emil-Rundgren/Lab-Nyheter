...
export default {
  data() {
    return {
      ...
      showAlert: false,
      category: 'sports' // This sets the category for this component for the address
    }
  },
  computed: {
    ...
    async getNews() {
      try {
        const response = await axios.get(
          'https://newsapi.org/v2/top-headlines?category=sports&country=us&apiKey=65198de0fc7e418eb3db417f3e104479'
        )
...
<template>
  ...
  <v-container fluid>
    ...
    <v-row v-for="(row, rowIndex) in sliceNewsItems" :key="`row-${rowIndex}`" justify="center">
      <!-- When screen size is less then 600px there will only be one news per row -->
      <v-col cols="12" sm="6" md="3" v-for="(item, index) in row" :key="`item-${index}`">
        ...
        <!-- : before to gives me the possebility to type JS in the address -->
